"""
@generated by mypy-protobuf.  Do not edit manually!
isort:skip_file
"""
import builtins
import collections.abc
import core.protos.base_pb2
import google.protobuf.descriptor
import google.protobuf.internal.containers
import google.protobuf.message
import google.protobuf.struct_pb2
import google.protobuf.wrappers_pb2
import sys

if sys.version_info >= (3, 8):
    import typing as typing_extensions
else:
    import typing_extensions

DESCRIPTOR: google.protobuf.descriptor.FileDescriptor

@typing_extensions.final
class LinearTeamAssetModel(google.protobuf.message.Message):
    """Simple Linear asset model using google.protobuf.Struct for flexible JSON storage
    This avoids complex proto definitions and allows flattened JSON display
    """

    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    ID_FIELD_NUMBER: builtins.int
    NAME_FIELD_NUMBER: builtins.int
    KEY_FIELD_NUMBER: builtins.int
    DESCRIPTION_FIELD_NUMBER: builtins.int
    METADATA_FIELD_NUMBER: builtins.int
    @property
    def id(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def name(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def key(self) -> google.protobuf.wrappers_pb2.StringValue:
        """Team key like "ENG" """
    @property
    def description(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def metadata(self) -> google.protobuf.struct_pb2.Struct:
        """Additional fields"""
    def __init__(
        self,
        *,
        id: google.protobuf.wrappers_pb2.StringValue | None = ...,
        name: google.protobuf.wrappers_pb2.StringValue | None = ...,
        key: google.protobuf.wrappers_pb2.StringValue | None = ...,
        description: google.protobuf.wrappers_pb2.StringValue | None = ...,
        metadata: google.protobuf.struct_pb2.Struct | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["description", b"description", "id", b"id", "key", b"key", "metadata", b"metadata", "name", b"name"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["description", b"description", "id", b"id", "key", b"key", "metadata", b"metadata", "name", b"name"]) -> None: ...

global___LinearTeamAssetModel = LinearTeamAssetModel

@typing_extensions.final
class LinearUserAssetModel(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    ID_FIELD_NUMBER: builtins.int
    NAME_FIELD_NUMBER: builtins.int
    EMAIL_FIELD_NUMBER: builtins.int
    DISPLAY_NAME_FIELD_NUMBER: builtins.int
    ACTIVE_FIELD_NUMBER: builtins.int
    TEAMS_FIELD_NUMBER: builtins.int
    METADATA_FIELD_NUMBER: builtins.int
    @property
    def id(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def name(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def email(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def display_name(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def active(self) -> google.protobuf.wrappers_pb2.BoolValue: ...
    @property
    def teams(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[global___LinearTeamReference]:
        """Teams this user belongs to"""
    @property
    def metadata(self) -> google.protobuf.struct_pb2.Struct:
        """Additional fields"""
    def __init__(
        self,
        *,
        id: google.protobuf.wrappers_pb2.StringValue | None = ...,
        name: google.protobuf.wrappers_pb2.StringValue | None = ...,
        email: google.protobuf.wrappers_pb2.StringValue | None = ...,
        display_name: google.protobuf.wrappers_pb2.StringValue | None = ...,
        active: google.protobuf.wrappers_pb2.BoolValue | None = ...,
        teams: collections.abc.Iterable[global___LinearTeamReference] | None = ...,
        metadata: google.protobuf.struct_pb2.Struct | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["active", b"active", "display_name", b"display_name", "email", b"email", "id", b"id", "metadata", b"metadata", "name", b"name"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["active", b"active", "display_name", b"display_name", "email", b"email", "id", b"id", "metadata", b"metadata", "name", b"name", "teams", b"teams"]) -> None: ...

global___LinearUserAssetModel = LinearUserAssetModel

@typing_extensions.final
class LinearProjectAssetModel(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    ID_FIELD_NUMBER: builtins.int
    NAME_FIELD_NUMBER: builtins.int
    DESCRIPTION_FIELD_NUMBER: builtins.int
    STATE_FIELD_NUMBER: builtins.int
    TEAMS_FIELD_NUMBER: builtins.int
    METADATA_FIELD_NUMBER: builtins.int
    @property
    def id(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def name(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def description(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def state(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def teams(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[global___LinearTeamReference]:
        """Teams this project belongs to"""
    @property
    def metadata(self) -> google.protobuf.struct_pb2.Struct:
        """Additional fields"""
    def __init__(
        self,
        *,
        id: google.protobuf.wrappers_pb2.StringValue | None = ...,
        name: google.protobuf.wrappers_pb2.StringValue | None = ...,
        description: google.protobuf.wrappers_pb2.StringValue | None = ...,
        state: google.protobuf.wrappers_pb2.StringValue | None = ...,
        teams: collections.abc.Iterable[global___LinearTeamReference] | None = ...,
        metadata: google.protobuf.struct_pb2.Struct | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["description", b"description", "id", b"id", "metadata", b"metadata", "name", b"name", "state", b"state"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["description", b"description", "id", b"id", "metadata", b"metadata", "name", b"name", "state", b"state", "teams", b"teams"]) -> None: ...

global___LinearProjectAssetModel = LinearProjectAssetModel

@typing_extensions.final
class LinearTeamReference(google.protobuf.message.Message):
    """Reference to a team (for team associations)"""

    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    ID_FIELD_NUMBER: builtins.int
    NAME_FIELD_NUMBER: builtins.int
    @property
    def id(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def name(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    def __init__(
        self,
        *,
        id: google.protobuf.wrappers_pb2.StringValue | None = ...,
        name: google.protobuf.wrappers_pb2.StringValue | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["id", b"id", "name", b"name"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["id", b"id", "name", b"name"]) -> None: ...

global___LinearTeamReference = LinearTeamReference

@typing_extensions.final
class LinearAssetModel(google.protobuf.message.Message):
    """Container for a single Linear asset"""

    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    ID_FIELD_NUMBER: builtins.int
    CONNECTOR_TYPE_FIELD_NUMBER: builtins.int
    TYPE_FIELD_NUMBER: builtins.int
    LAST_UPDATED_FIELD_NUMBER: builtins.int
    NAME_FIELD_NUMBER: builtins.int
    LINEAR_ID_FIELD_NUMBER: builtins.int
    LINEAR_TEAM_FIELD_NUMBER: builtins.int
    LINEAR_USER_FIELD_NUMBER: builtins.int
    LINEAR_PROJECT_FIELD_NUMBER: builtins.int
    @property
    def id(self) -> google.protobuf.wrappers_pb2.UInt64Value: ...
    connector_type: core.protos.base_pb2.Source.ValueType
    type: core.protos.base_pb2.SourceModelType.ValueType
    last_updated: builtins.int
    @property
    def name(self) -> google.protobuf.wrappers_pb2.StringValue:
        """Display name for the asset (team name, user name, or project name)"""
    @property
    def linear_id(self) -> google.protobuf.wrappers_pb2.StringValue:
        """Linear resource ID (team id, user id, or project id)"""
    @property
    def linear_team(self) -> global___LinearTeamAssetModel: ...
    @property
    def linear_user(self) -> global___LinearUserAssetModel: ...
    @property
    def linear_project(self) -> global___LinearProjectAssetModel: ...
    def __init__(
        self,
        *,
        id: google.protobuf.wrappers_pb2.UInt64Value | None = ...,
        connector_type: core.protos.base_pb2.Source.ValueType = ...,
        type: core.protos.base_pb2.SourceModelType.ValueType = ...,
        last_updated: builtins.int = ...,
        name: google.protobuf.wrappers_pb2.StringValue | None = ...,
        linear_id: google.protobuf.wrappers_pb2.StringValue | None = ...,
        linear_team: global___LinearTeamAssetModel | None = ...,
        linear_user: global___LinearUserAssetModel | None = ...,
        linear_project: global___LinearProjectAssetModel | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["id", b"id", "linear", b"linear", "linear_id", b"linear_id", "linear_project", b"linear_project", "linear_team", b"linear_team", "linear_user", b"linear_user", "name", b"name"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["connector_type", b"connector_type", "id", b"id", "last_updated", b"last_updated", "linear", b"linear", "linear_id", b"linear_id", "linear_project", b"linear_project", "linear_team", b"linear_team", "linear_user", b"linear_user", "name", b"name", "type", b"type"]) -> None: ...
    def WhichOneof(self, oneof_group: typing_extensions.Literal["linear", b"linear"]) -> typing_extensions.Literal["linear_team", "linear_user", "linear_project"] | None: ...

global___LinearAssetModel = LinearAssetModel

@typing_extensions.final
class LinearAssets(google.protobuf.message.Message):
    """Container for all Linear assets"""

    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    ASSETS_FIELD_NUMBER: builtins.int
    @property
    def assets(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[global___LinearAssetModel]: ...
    def __init__(
        self,
        *,
        assets: collections.abc.Iterable[global___LinearAssetModel] | None = ...,
    ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal["assets", b"assets"]) -> None: ...

global___LinearAssets = LinearAssets

@typing_extensions.final
class LinearTeamAssetOptions(google.protobuf.message.Message):
    """Filter options for Linear assets"""

    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    TEAM_IDS_FIELD_NUMBER: builtins.int
    TEAM_NAMES_FIELD_NUMBER: builtins.int
    @property
    def team_ids(self) -> google.protobuf.internal.containers.RepeatedScalarFieldContainer[builtins.str]: ...
    @property
    def team_names(self) -> google.protobuf.internal.containers.RepeatedScalarFieldContainer[builtins.str]: ...
    def __init__(
        self,
        *,
        team_ids: collections.abc.Iterable[builtins.str] | None = ...,
        team_names: collections.abc.Iterable[builtins.str] | None = ...,
    ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal["team_ids", b"team_ids", "team_names", b"team_names"]) -> None: ...

global___LinearTeamAssetOptions = LinearTeamAssetOptions

@typing_extensions.final
class LinearUserAssetOptions(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    USER_IDS_FIELD_NUMBER: builtins.int
    TEAM_IDS_FIELD_NUMBER: builtins.int
    @property
    def user_ids(self) -> google.protobuf.internal.containers.RepeatedScalarFieldContainer[builtins.str]: ...
    @property
    def team_ids(self) -> google.protobuf.internal.containers.RepeatedScalarFieldContainer[builtins.str]:
        """Filter by team membership"""
    def __init__(
        self,
        *,
        user_ids: collections.abc.Iterable[builtins.str] | None = ...,
        team_ids: collections.abc.Iterable[builtins.str] | None = ...,
    ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal["team_ids", b"team_ids", "user_ids", b"user_ids"]) -> None: ...

global___LinearUserAssetOptions = LinearUserAssetOptions

@typing_extensions.final
class LinearProjectAssetOptions(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    PROJECT_IDS_FIELD_NUMBER: builtins.int
    TEAM_IDS_FIELD_NUMBER: builtins.int
    STATES_FIELD_NUMBER: builtins.int
    @property
    def project_ids(self) -> google.protobuf.internal.containers.RepeatedScalarFieldContainer[builtins.str]: ...
    @property
    def team_ids(self) -> google.protobuf.internal.containers.RepeatedScalarFieldContainer[builtins.str]:
        """Filter by team association"""
    @property
    def states(self) -> google.protobuf.internal.containers.RepeatedScalarFieldContainer[builtins.str]:
        """Filter by project state"""
    def __init__(
        self,
        *,
        project_ids: collections.abc.Iterable[builtins.str] | None = ...,
        team_ids: collections.abc.Iterable[builtins.str] | None = ...,
        states: collections.abc.Iterable[builtins.str] | None = ...,
    ) -> None: ...
    def ClearField(self, field_name: typing_extensions.Literal["project_ids", b"project_ids", "states", b"states", "team_ids", b"team_ids"]) -> None: ...

global___LinearProjectAssetOptions = LinearProjectAssetOptions
