"""
@generated by mypy-protobuf.  Do not edit manually!
isort:skip_file
"""
import builtins
import collections.abc
import core.protos.assets.asset_pb2
import core.protos.base_pb2
import core.protos.connectors.api_pb2
import core.protos.connectors.connector_pb2
import google.protobuf.descriptor
import google.protobuf.internal.containers
import google.protobuf.message
import google.protobuf.struct_pb2
import google.protobuf.wrappers_pb2
import sys

if sys.version_info >= (3, 8):
    import typing as typing_extensions
else:
    import typing_extensions

DESCRIPTOR: google.protobuf.descriptor.FileDescriptor

@typing_extensions.final
class GetConnectorsAssetsModelsOptionsRequest(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    META_FIELD_NUMBER: builtins.int
    CONNECTOR_TYPE_FIELD_NUMBER: builtins.int
    CONNECTOR_ID_FIELD_NUMBER: builtins.int
    MODEL_TYPE_FIELD_NUMBER: builtins.int
    @property
    def meta(self) -> core.protos.connectors.api_pb2.Meta: ...
    connector_type: core.protos.base_pb2.Source.ValueType
    @property
    def connector_id(self) -> google.protobuf.wrappers_pb2.UInt64Value: ...
    model_type: core.protos.base_pb2.SourceModelType.ValueType
    def __init__(
        self,
        *,
        meta: core.protos.connectors.api_pb2.Meta | None = ...,
        connector_type: core.protos.base_pb2.Source.ValueType = ...,
        connector_id: google.protobuf.wrappers_pb2.UInt64Value | None = ...,
        model_type: core.protos.base_pb2.SourceModelType.ValueType = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["connector_id", b"connector_id", "meta", b"meta"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["connector_id", b"connector_id", "connector_type", b"connector_type", "meta", b"meta", "model_type", b"model_type"]) -> None: ...

global___GetConnectorsAssetsModelsOptionsRequest = GetConnectorsAssetsModelsOptionsRequest

@typing_extensions.final
class GetConnectorsAssetsModelsOptionsResponse(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    META_FIELD_NUMBER: builtins.int
    SUCCESS_FIELD_NUMBER: builtins.int
    MESSAGE_FIELD_NUMBER: builtins.int
    ASSET_MODEL_OPTIONS_FIELD_NUMBER: builtins.int
    @property
    def meta(self) -> core.protos.connectors.api_pb2.Meta: ...
    @property
    def success(self) -> google.protobuf.wrappers_pb2.BoolValue: ...
    @property
    def message(self) -> core.protos.connectors.api_pb2.Message: ...
    @property
    def asset_model_options(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[core.protos.assets.asset_pb2.AccountConnectorAssetsModelOptions]: ...
    def __init__(
        self,
        *,
        meta: core.protos.connectors.api_pb2.Meta | None = ...,
        success: google.protobuf.wrappers_pb2.BoolValue | None = ...,
        message: core.protos.connectors.api_pb2.Message | None = ...,
        asset_model_options: collections.abc.Iterable[core.protos.assets.asset_pb2.AccountConnectorAssetsModelOptions] | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["message", b"message", "meta", b"meta", "success", b"success"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["asset_model_options", b"asset_model_options", "message", b"message", "meta", b"meta", "success", b"success"]) -> None: ...

global___GetConnectorsAssetsModelsOptionsResponse = GetConnectorsAssetsModelsOptionsResponse

@typing_extensions.final
class GetConnectorsAssetsModelsRequest(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    META_FIELD_NUMBER: builtins.int
    CONNECTOR_ID_FIELD_NUMBER: builtins.int
    CONNECTOR_TYPE_FIELD_NUMBER: builtins.int
    TYPE_FIELD_NUMBER: builtins.int
    FILTERS_FIELD_NUMBER: builtins.int
    PAGE_FIELD_NUMBER: builtins.int
    PAGE_SIZE_FIELD_NUMBER: builtins.int
    @property
    def meta(self) -> core.protos.connectors.api_pb2.Meta: ...
    @property
    def connector_id(self) -> google.protobuf.wrappers_pb2.UInt64Value: ...
    connector_type: core.protos.base_pb2.Source.ValueType
    type: core.protos.base_pb2.SourceModelType.ValueType
    @property
    def filters(self) -> core.protos.assets.asset_pb2.AccountConnectorAssetsModelFilters: ...
    @property
    def page(self) -> google.protobuf.wrappers_pb2.UInt32Value: ...
    @property
    def page_size(self) -> google.protobuf.wrappers_pb2.UInt32Value: ...
    def __init__(
        self,
        *,
        meta: core.protos.connectors.api_pb2.Meta | None = ...,
        connector_id: google.protobuf.wrappers_pb2.UInt64Value | None = ...,
        connector_type: core.protos.base_pb2.Source.ValueType = ...,
        type: core.protos.base_pb2.SourceModelType.ValueType = ...,
        filters: core.protos.assets.asset_pb2.AccountConnectorAssetsModelFilters | None = ...,
        page: google.protobuf.wrappers_pb2.UInt32Value | None = ...,
        page_size: google.protobuf.wrappers_pb2.UInt32Value | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["connector_id", b"connector_id", "filters", b"filters", "meta", b"meta", "page", b"page", "page_size", b"page_size"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["connector_id", b"connector_id", "connector_type", b"connector_type", "filters", b"filters", "meta", b"meta", "page", b"page", "page_size", b"page_size", "type", b"type"]) -> None: ...

global___GetConnectorsAssetsModelsRequest = GetConnectorsAssetsModelsRequest

@typing_extensions.final
class ApiGetConnectorsAssetsModelsRequest(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    META_FIELD_NUMBER: builtins.int
    CONNECTOR_ID_FIELD_NUMBER: builtins.int
    TASK_TYPE_FIELD_NUMBER: builtins.int
    FILTERS_FIELD_NUMBER: builtins.int
    @property
    def meta(self) -> core.protos.connectors.api_pb2.Meta: ...
    @property
    def connector_id(self) -> google.protobuf.wrappers_pb2.UInt64Value: ...
    @property
    def task_type(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def filters(self) -> google.protobuf.struct_pb2.Struct: ...
    def __init__(
        self,
        *,
        meta: core.protos.connectors.api_pb2.Meta | None = ...,
        connector_id: google.protobuf.wrappers_pb2.UInt64Value | None = ...,
        task_type: google.protobuf.wrappers_pb2.StringValue | None = ...,
        filters: google.protobuf.struct_pb2.Struct | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["connector_id", b"connector_id", "filters", b"filters", "meta", b"meta", "task_type", b"task_type"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["connector_id", b"connector_id", "filters", b"filters", "meta", b"meta", "task_type", b"task_type"]) -> None: ...

global___ApiGetConnectorsAssetsModelsRequest = ApiGetConnectorsAssetsModelsRequest

@typing_extensions.final
class ModelTypeCount(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    MODEL_TYPE_FIELD_NUMBER: builtins.int
    TOTAL_COUNT_FIELD_NUMBER: builtins.int
    model_type: core.protos.base_pb2.SourceModelType.ValueType
    @property
    def total_count(self) -> google.protobuf.wrappers_pb2.UInt32Value: ...
    def __init__(
        self,
        *,
        model_type: core.protos.base_pb2.SourceModelType.ValueType = ...,
        total_count: google.protobuf.wrappers_pb2.UInt32Value | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["total_count", b"total_count"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["model_type", b"model_type", "total_count", b"total_count"]) -> None: ...

global___ModelTypeCount = ModelTypeCount

@typing_extensions.final
class GetConnectorsAssetsModelsResponse(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    META_FIELD_NUMBER: builtins.int
    SUCCESS_FIELD_NUMBER: builtins.int
    MESSAGE_FIELD_NUMBER: builtins.int
    ASSETS_FIELD_NUMBER: builtins.int
    ASSET_DESCRIPTOR_FIELD_NUMBER: builtins.int
    MODEL_TYPE_COUNTS_FIELD_NUMBER: builtins.int
    CURRENT_PAGE_FIELD_NUMBER: builtins.int
    TOTAL_PAGES_FIELD_NUMBER: builtins.int
    @property
    def meta(self) -> core.protos.connectors.api_pb2.Meta: ...
    @property
    def success(self) -> google.protobuf.wrappers_pb2.BoolValue: ...
    @property
    def message(self) -> core.protos.connectors.api_pb2.Message: ...
    @property
    def assets(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[core.protos.assets.asset_pb2.AccountConnectorAssets]: ...
    @property
    def asset_descriptor(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def model_type_counts(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[global___ModelTypeCount]: ...
    @property
    def current_page(self) -> google.protobuf.wrappers_pb2.UInt32Value: ...
    @property
    def total_pages(self) -> google.protobuf.wrappers_pb2.UInt32Value: ...
    def __init__(
        self,
        *,
        meta: core.protos.connectors.api_pb2.Meta | None = ...,
        success: google.protobuf.wrappers_pb2.BoolValue | None = ...,
        message: core.protos.connectors.api_pb2.Message | None = ...,
        assets: collections.abc.Iterable[core.protos.assets.asset_pb2.AccountConnectorAssets] | None = ...,
        asset_descriptor: google.protobuf.wrappers_pb2.StringValue | None = ...,
        model_type_counts: collections.abc.Iterable[global___ModelTypeCount] | None = ...,
        current_page: google.protobuf.wrappers_pb2.UInt32Value | None = ...,
        total_pages: google.protobuf.wrappers_pb2.UInt32Value | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["asset_descriptor", b"asset_descriptor", "current_page", b"current_page", "message", b"message", "meta", b"meta", "success", b"success", "total_pages", b"total_pages"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["asset_descriptor", b"asset_descriptor", "assets", b"assets", "current_page", b"current_page", "message", b"message", "meta", b"meta", "model_type_counts", b"model_type_counts", "success", b"success", "total_pages", b"total_pages"]) -> None: ...

global___GetConnectorsAssetsModelsResponse = GetConnectorsAssetsModelsResponse

@typing_extensions.final
class GetConnectorsAssetsModelsRefreshRequest(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    META_FIELD_NUMBER: builtins.int
    CONNECTOR_ID_FIELD_NUMBER: builtins.int
    @property
    def meta(self) -> core.protos.connectors.api_pb2.Meta: ...
    @property
    def connector_id(self) -> google.protobuf.wrappers_pb2.UInt64Value: ...
    def __init__(
        self,
        *,
        meta: core.protos.connectors.api_pb2.Meta | None = ...,
        connector_id: google.protobuf.wrappers_pb2.UInt64Value | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["connector_id", b"connector_id", "meta", b"meta"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["connector_id", b"connector_id", "meta", b"meta"]) -> None: ...

global___GetConnectorsAssetsModelsRefreshRequest = GetConnectorsAssetsModelsRefreshRequest

@typing_extensions.final
class GetConnectorsAssetsModelsRefreshResponse(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    META_FIELD_NUMBER: builtins.int
    SUCCESS_FIELD_NUMBER: builtins.int
    MESSAGE_FIELD_NUMBER: builtins.int
    @property
    def meta(self) -> core.protos.connectors.api_pb2.Meta: ...
    @property
    def success(self) -> google.protobuf.wrappers_pb2.BoolValue: ...
    @property
    def message(self) -> core.protos.connectors.api_pb2.Message: ...
    def __init__(
        self,
        *,
        meta: core.protos.connectors.api_pb2.Meta | None = ...,
        success: google.protobuf.wrappers_pb2.BoolValue | None = ...,
        message: core.protos.connectors.api_pb2.Message | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["message", b"message", "meta", b"meta", "success", b"success"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["message", b"message", "meta", b"meta", "success", b"success"]) -> None: ...

global___GetConnectorsAssetsModelsRefreshResponse = GetConnectorsAssetsModelsRefreshResponse

@typing_extensions.final
class ProxyConnectorRegisterConnectorMetadataRequest(google.protobuf.message.Message):
    """///////////////////  Proxy Connector Asset PIs schema  /////////////////////
    When sending batched assets (e.g. 100 per request), send the same refresh_id
    for all batches and set has_more=true until the last batch (has_more=false).
    Backend will accumulate and run full-set sync only when has_more=false.
    """

    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    CONNECTOR_FIELD_NUMBER: builtins.int
    ASSETS_FIELD_NUMBER: builtins.int
    REFRESH_ID_FIELD_NUMBER: builtins.int
    HAS_MORE_FIELD_NUMBER: builtins.int
    MODEL_TYPE_FIELD_NUMBER: builtins.int
    @property
    def connector(self) -> core.protos.connectors.connector_pb2.Connector: ...
    @property
    def assets(self) -> google.protobuf.internal.containers.RepeatedCompositeFieldContainer[core.protos.assets.asset_pb2.AssetMetadataModel]: ...
    @property
    def refresh_id(self) -> google.protobuf.wrappers_pb2.StringValue: ...
    @property
    def has_more(self) -> google.protobuf.wrappers_pb2.BoolValue: ...
    model_type: core.protos.base_pb2.SourceModelType.ValueType
    """model_type this batch belongs to"""
    def __init__(
        self,
        *,
        connector: core.protos.connectors.connector_pb2.Connector | None = ...,
        assets: collections.abc.Iterable[core.protos.assets.asset_pb2.AssetMetadataModel] | None = ...,
        refresh_id: google.protobuf.wrappers_pb2.StringValue | None = ...,
        has_more: google.protobuf.wrappers_pb2.BoolValue | None = ...,
        model_type: core.protos.base_pb2.SourceModelType.ValueType = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["connector", b"connector", "has_more", b"has_more", "refresh_id", b"refresh_id"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["assets", b"assets", "connector", b"connector", "has_more", b"has_more", "model_type", b"model_type", "refresh_id", b"refresh_id"]) -> None: ...

global___ProxyConnectorRegisterConnectorMetadataRequest = ProxyConnectorRegisterConnectorMetadataRequest

@typing_extensions.final
class ProxyConnectorRegisterConnectorMetadataResponse(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    SUCCESS_FIELD_NUMBER: builtins.int
    MESSAGE_FIELD_NUMBER: builtins.int
    @property
    def success(self) -> google.protobuf.wrappers_pb2.BoolValue: ...
    @property
    def message(self) -> core.protos.connectors.api_pb2.Message: ...
    def __init__(
        self,
        *,
        success: google.protobuf.wrappers_pb2.BoolValue | None = ...,
        message: core.protos.connectors.api_pb2.Message | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing_extensions.Literal["message", b"message", "success", b"success"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing_extensions.Literal["message", b"message", "success", b"success"]) -> None: ...

global___ProxyConnectorRegisterConnectorMetadataResponse = ProxyConnectorRegisterConnectorMetadataResponse
