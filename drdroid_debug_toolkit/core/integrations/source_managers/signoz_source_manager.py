...
class SignozSourceManager(SourceManager):
    def __init__(self):
        ...
        self.task_type_callable_map = {
            ...
            Signoz.TaskType.FETCH_DASHBOARD_VARIABLES: {
                "executor": self.execute_fetch_dashboard_variables,
                "model_types": [],
                "result_type": PlaybookTaskResultType.API_RESPONSE,
                "display_name": "Fetch Dashboard Variables",
                "category": "Dashboard",
                "form_fields": [
                    FormField(
                        key_name=StringValue(value="dashboard_id"),
                        display_name=StringValue(value="Dashboard ID or Name"),
                        description=StringValue(value="Dashboard ID or title to fetch variables from"),
                        data_type=LiteralType.STRING,
                        form_field_type=FormFieldType.TEXT_FT,
                    ),
                    FormField(
                        key_name=StringValue(value="variables_json"),
                        display_name=StringValue(value="Variable Overrides (JSON)"),
                        description=StringValue(value='Optional JSON object to override specific variables, e.g. {"env":"production"}'),
                        data_type=LiteralType.STRING,
                        is_optional=True,
                        default_value=Literal(
                            type=LiteralType.STRING,
                    ...
                    FormField(
                        key_name=StringValue(value="duration"),
                        display_name=StringValue(value="Duration"),
                        description=StringValue(value="Lookback window for resolving QUERY variables (e.g. '3h', '30m'). Defaults to 3h."),
                        data_type=LiteralType.STRING,
                        is_optional=True,
                        default_value=Literal(
                            type=LiteralType.STRING,
                            string=StringValue(value="3h"),
                        ),
                        form_field_type=FormFieldType.TEXT_FT,
                    ),
                ],
    ...
    def execute_fetch_dashboard_variables(self, time_range: TimeRange, signoz_task: Signoz,
                                          signoz_connector: ConnectorProto):
        """Fetch and resolve all variables for a SigNoz dashboard, including QUERY→QUERY chains."""
        try:
            if not signoz_connector:
                return PlaybookTaskResult(
                    type=PlaybookTaskResultType.TEXT,
                    text=TextResult(output=StringValue(
                        value="Task execution Failed:: No SigNoz source found")),
                    source=self.source,
                )

            task = signoz_task.fetch_dashboard_variables
            dashboard_id = task.dashboard_id.value if task.HasField("dashboard_id") else ""
            variables_json = task.variables_json.value if task.HasField("variables_json") else None
            duration = task.duration.value if task.HasField("duration") else "3h"

            if not dashboard_id:
                return PlaybookTaskResult(
                    type=PlaybookTaskResultType.TEXT,
                    text=TextResult(output=StringValue(
                        value="dashboard_id is required for FETCH_DASHBOARD_VARIABLES")),
                    source=self.source,
                )
            ...
            print(
                f"Playbook Task Downstream Request: Type -> Signoz FETCH_DASHBOARD_VARIABLES, "
                f"Dashboard -> {dashboard_id}",
                flush=True,
            )

            variables_data = signoz_api_processor.get_dashboard_variables(
                dashboard_id, resolve_queries=True, duration=duration
            )

            # Apply user overrides when provided
            if variables_json:
                try:
                    overrides = json.loads(variables_json)
                    if isinstance(overrides, dict):
                        for var_name, override_val in overrides.items():
                            if var_name in variables_data.get("variables", {}):
                                variables_data["variables"][var_name]["selected_value"] = override_val
                            else:
                                variables_data.setdefault("variables", {})[var_name] = {
                                    "type": "OVERRIDE",
                                    "selected_value": override_val,
                                    "allowed_values": [override_val] if not isinstance(override_val, list) else override_val,
                                    "query": "",
                                }
                except Exception as parse_err:
                    logger.warning(f"Failed to parse variables_json override: {parse_err}")

            if not variables_data or not variables_data.get("variables"):
                return PlaybookTaskResult(
                    type=PlaybookTaskResultType.TEXT,
                    text=TextResult(output=StringValue(
                        value=f"No variables found for dashboard: {dashboard_id}")),
                    source=self.source,
                    metadata=metadata,
                )
            ...
            logger.error(f"Error while executing SigNoz fetch_dashboard_variables task: {e}", exc_info=True)
            api_url = self._extract_api_url_from_connector(signoz_connector) if signoz_connector else None
            metadata = self._create_metadata_with_signoz_url(api_url, "dashboards", {})
            return PlaybookTaskResult(
                type=PlaybookTaskResultType.TEXT,
                text=TextResult(output=StringValue(
                    value=f"Error fetching dashboard variables: {str(e)}")),
                source=self.source,
                metadata=metadata,
            )
