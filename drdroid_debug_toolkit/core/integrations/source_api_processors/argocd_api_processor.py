    ...
    def get_deployment_info(self):
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"Error occurred while fetching deployment info from argocd "
    ...
    def get_application_details(self, app_name):
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"Error occurred while fetching application details from argocd "
                    f"with status_code: {response.status_code} and response: {response.text}"
                )
        except Exception as e:
    ...
    def disable_auto_sync(self, app_name):
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"Error occurred while disabling auto-sync for {app_name} from argocd "
                    f"with status_code: {response.status_code} and response: {response.text}"
                )
        except Exception as e:
    ...
    def update_application_revision(self, app_name, target_revision, deployment_id):
        ...
        try:
            ...
            if response.status_code in (200, 204):
                ...
                raise Exception(
                    f"Error occurred while updating target revision in ArgoCD with status_code: {response.status_code} and response: {response.text}"
                )
        except Exception as e:
            logger.error(f"Exception occurred while updating target revision for application '{app_name}' with error: {e}")
            raise e
    ...
    def get_application_health(self, app_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {"status": health.get("status", "Unknown"), "sync_status": app_data.get("status", {}).get("sync", {}).get("status", "Unknown")}
            else:
