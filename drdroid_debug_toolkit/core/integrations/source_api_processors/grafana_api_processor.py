...
class GrafanaApiProcessor(Processor):
    ...
    def fetch_alert_state_history(self, rule_uid, start_time=None, end_time=None, duration=None):
        """
        ...
        Parameters
        ----------
        rule_uid   : str  Alert rule UID (required)
        start_time : str  RFC3339 or epoch-ms (optional)
        end_time   : str  RFC3339 or epoch-ms (optional)
        duration   : str  Duration window, e.g. '24h', '7d'. Defaults to last 24 h.
        """
        if not rule_uid:
            raise ValueError("rule_uid is required for fetch_alert_state_history")

        # Resolve time window
        now = datetime.datetime.now(datetime.timezone.utc)
        if duration:
            try:
                hours = _parse_duration_to_hours(duration)
                end_dt = now
            ...
            end_dt = _parse_time_string(end_time) if end_time else now
            start_dt = _parse_time_string(start_time) if start_time else (now - datetime.timedelta(hours=24))
