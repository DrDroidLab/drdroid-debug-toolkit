...
class DatadogOauthApiProcessor(Processor):
    ...
    def fetch_metric_timeseries(self, time_geq, time_lt, specific_metric, interval=300000):
        metric_queries = specific_metric.get('queries', None)
        formulas = specific_metric.get('formulas', None)
        from_tr = int(time_geq * 1000)
        to_tr = int(time_lt * 1000)
