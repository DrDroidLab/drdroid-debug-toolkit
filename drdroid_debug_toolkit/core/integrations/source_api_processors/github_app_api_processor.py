...
class GithubAppAPIProcessor(Processor):
    def __init__(self, app_id, private_key, installation_id, org=None):
        self.app_id = app_id
    ...
    def _get_installation_token(self):
        ...
        try:
            ...
            response = requests.post(url, headers=headers, timeout=EXTERNAL_CALL_TIMEOUT)
            response.raise_for_status()
            
            token_data = response.json()
            self._installation_token = token_data['token']
            
            # Cache expiration time (tokens typically expire in 1 hour)
    ...
    def test_connection(self):
        try:
            ...
            logger.error(f"GithubAppAPIProcessor.test_connection:: Exception occurred with error: {e}")
            raise e
        ...
        try:
            ...
            if response:
                if response.status_code == 200:
                    ...
                    logger.error(f"GithubAppAPIProcessor.get_file_commits:: Error occurred while fetching github commit "
                                 f"details for file: {file_path} in {self.org}/{repo}/{branch} with status_code: "
                                 f"{response.status_code} and response: {response.text}")
        except Exception as e:
            logger.error(f"GithubAppAPIProcessor.get_file_commits:: Exception occurred while fetching github commit issue "
        ...
        try:
            ...
            if response:
                if response.status_code == 200:
                    ...
                    logger.error(f"GithubAppAPIProcessor.get_commit_sha:: Error occurred while fetching github commit "
                                 f"details for commit_sha: {commit_sha} in {self.org}/{repo} with status_code: "
                                 f"{response.status_code} and response: {response.text}")
        except Exception as e:
            logger.error(f"GithubAppAPIProcessor.get_commit_sha:: Exception occurred while fetching github commit details "
        ...
        try:
            ...
            if branch_name:
                ...
                if not created_branch:
                    logger.error(f"GithubAppAPIProcessor.update_file:: Error occurred while creating branch: {branch_name}"
                                 f" in {self.org}/{repo}")
                    return None
            ...
            if response.status_code == 200:
                ...
                logger.error(f"GithubAppAPIProcessor.update_file:: Error occurred while updating github file details for "
                             f"file: {file_path} in {self.org}/{repo} with status_code: {response.status_code} and "
                             f"response: {response.text}")
        except Exception as e:
            logger.error(f"GithubAppAPIProcessor.update_file:: Exception occurred while updating github file details for "
                         f"file: {file_path} in {self.org}/{repo} with error: {e}")
        return None
        ...
        try:
            ...
            while True:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"GithubAppAPIProcessor.get_branch:: Error occurred while getting all github branches "
                                 f"in {self.org}/{repo} with status_code: {response.status_code} "
                                 f"and response: {response.text}")
                break
            return all_branches
        except Exception as e:
            logger.error(f"GithubAppAPIProcessor.get_branch:: Exception occurred while getting all github branches "
    ...
    def search_file(self, repo, file_path):
        ...
        try:
            ...
            logger.error(f"GithubAppAPIProcessor.search_file:: Exception occurred while searching for file: "
    ...
    def get_branch(self, repo, branch_name):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"GithubAppAPIProcessor.get_branch:: Error occurred while getting github branch details for "
                             f"branch: {branch_name} in {self.org}/{repo} with status_code: {response.status_code} and "
                             f"response: {response.text}")
        except Exception as e:
            logger.error(f"GithubAppAPIProcessor.get_branch:: Exception occurred while getting github branch details for "
    ...
    def create_branch(self, repo, branch_name):
        try:
            ...
            if is_existing:
                logger.warning(f"GithubAppAPIProcessor.create_branch:: Branch {branch_name} already exists in "
                               f"{self.org}/{repo}")
                return is_existing
            ...
            for b in all_branches:
                if b['name'] in main_branches:
                    ...
                    break
            if not main_branch:
                logger.error(f"GithubAppAPIProcessor.create_branch:: Main branch not found in {self.org}/{repo}")
                return None
            ...
            if response.status_code == 201:
                ...
                logger.error(f"GithubAppAPIProcessor.create_branch:: Error occurred while creating github branch: "
                             f"{branch_name} in {self.org}/{repo} with status_code: {response.status_code} and "
                             f"response: {response.text}")
        except Exception as e:
            logger.error(f"GithubAppAPIProcessor.create_branch:: Exception occurred while creating github branch: "
        ...
        try:
            ...
            if response:
                if response.status_code == 200:
                    ...
                    logger.error(f"GithubAppAPIProcessor.get_branch_commits:: Error occurred while fetching github commit "
                                 f"details for branch: {branch} in {self.org}/{repo} with status_code: "
                                 f"{response.status_code} and response: {response.text}")
        except Exception as e:
            logger.error(
                f"GithubAppAPIProcessor.get_branch_commits:: Exception occurred while fetching github commit issue "
        ...
        try:
            ...
            if response:
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"GithubAppAPIProcessor.get_recent_merges:: Error occurred while fetching github PR merges "
                        f"details for branch: {branch} in {self.org}/{repo} with status_code: "
                        f"{response.status_code} and response: {response.text}")
        except Exception as e:
            logger.error(f"GithubAppAPIProcessor.get_recent_merges:: Exception occurred while fetching github PR merges "
                         f"details for branch: {branch} in {self.org}/{repo} with error: {e}")
        return None
